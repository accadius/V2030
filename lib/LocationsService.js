...
function LocationsService() {
	...
	parseCSV = function(text) {
		...
		var link, name,thumbURL,url,description,color,pt,pms,attr,graphic;
		...
		for (var i = 1; i < lines.length; i++) {
			...
			link = values[fields.indexOf("Link")];
			...
			(color == 'G')
				pms = new esri.symbol.PictureMarkerSymbol("images/icons/green/NumberIcong"+(i)+".png",22,28);			
			else
				pms = new esri.symbol.PictureMarkerSymbol("images/icons/red/NumberIcon"+(i)+".png",22,28);
				
			pms.setOffset(3,8);
			attr = {link:link,name:name,thumbURL:thumbURL,url:url,description:description,color:color};
